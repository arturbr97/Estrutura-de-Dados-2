...
void Ins(TipoRegistro Reg, TipoApontador Ap, short *Cresceu, TipoRegistro *RegRetorno, TipoApontador *ApRetorno, int ordemArvore)
{
    ...
    Ap->n = ordemArvore;
    ApTemp->p[0] = Ap->p[ordemArvore + 1];
    *RegRetorno = Ap->r[ordemArvore];
    *ApRetorno = ApTemp;
}

void InsereArvore(TipoRegistro Reg, TipoApontador *Ap, int ordemArvore)
{
    short Cresceu;
...
void Antecessor(TipoApontador Ap, int Ind, TipoApontador ApPai, short *Diminuiu, int ordemArvore)
{
    ...
    ApPai->n--;
    *Diminuiu = (ApPai->n < ordemArvore);
}

void Ret(int Ch, TipoApontador *Ap, short *Diminuiu, int ordemArvore)
{
    long j, Ind = 1;
    TipoApontador Pag;
    if (*Ap == NULL)
...
void Retira(int Ch, TipoApontador *Ap, int ordemArvore)
{
    short Diminuiu;
    TipoApontador Aux;
